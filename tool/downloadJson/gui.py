...
def execute_script():
    ...
    try:
        ...
        # convert.py をスクリプトディレクトリで実行
        subprocess.run(["python", script_path, selected_url, selected_name, selected_sheet], check=True, cwd=script_dir)
        messagebox.showinfo("成功", f"{selected_name} のjsonを作成しました")
    except subprocess.CalledProcessError:
        messagebox.showerror("エラー", "スクリプトの実行に失敗しました")

# GUIの作成
root = tk.Tk()
root.title("json作成ツール")
